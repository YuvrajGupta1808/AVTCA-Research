...
calib() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PY" "$WT/scripts/calibrate_engagement_logits.py" \
    "${CAL[@]}" --checkpoint_path "$d/ENGAGENET_multimodal_cnn_15_best.pth" \
    --result_path "$d/calib" "$@" > "$d/calib.log" 2>&1
  echo "$name calib rc=$?" >> "$OUT/calib_status.txt"
}
cd "$WT"
calib 0 WB_behavior_text --behavior --text_fusion --behavior_dir "$BEH" &
calib 1 WA_control &
wait
echo "CALIBRATION DONE" >> "$OUT/calib_status.txt"
