...
def parse_args():
    ...
    # Text defaults must match src/config/opts.py training defaults (32/4096):
    # the hash-bucket tokenizer maps words to different ids under a different
    # vocab size, which would silently feed the model wrong text at calibration.
    parser.add_argument('--max_text_tokens', default=32, type=int)
    parser.add_argument('--text_vocab_size', default=4096, type=int)
    ...
    parser.add_argument('--visual_features', default='frames', choices=['frames', 'marlin'])
    parser.add_argument('--marlin_root', default='')
    parser.add_argument('--marlin_tokens', default=9, type=int)
